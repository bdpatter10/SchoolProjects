...
public class ExpressionEvaluator
{
   ...
   public ExpressionEvaluator(String Expression, String Type)
   {
         ...
         for (int i = 0; i < Nodes.size(); ++i)
         {
            switch (Nodes.get(i).toString().charAt(0))
            {
               ...
               case '^':
                  ...
                  while ((OperationStack.size() > 0) && (OperationStack.get(0).toString().equals("(") == false) && 
                		  (ExpressionEvaluator.Precedence(Nodes.get(i).toString()) <= ExpressionEvaluator.Precedence(OperationStack.get(0).toString())))
                  {
                     ValueStack.add(0, OperationStack.remove(0));
                     ValueStack.get(0).SetChildren(ValueStack.remove(2), ValueStack.remove(1));
                  }
