   ...
   public String toString()
   {
	...
	public int compareTo(Course course) 
	{
		...
		if (!prefix.equals(course.getPrefix()))
		{
			...
			int l2 = course.prefix.length();
			result = l1 - l2;
			if(l1 == l2)
			{
				while(prefix.length() > index1)
				{
					value1 += prefix.charAt(index1);
					index1++;
				}
				while(course.getPrefix().length() > index2)
				{
					value2 += course.getPrefix().charAt(index2);
					index2++;
				}
